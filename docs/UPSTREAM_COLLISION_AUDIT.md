...
| 2 | **gpt-5.5/5.5-pro 按 5.4 价计费**(billing R9) | 兜底路径 5.5 按 $2.5/$15(Fast 2.5x),官方 $5/$30(Fast 2x) | **跟随上游独立价卡**;若定价目录已含 5.5 官方价则此表不生效,可仅跟数据 | 潜在收入损失 2x;实施前先验证 Nova 目录是否覆盖 5.5;成本低 |
...
| 8 | **composite 平台收紧 + 迁移 227**(GroupsView/ent) | 前端白名单 5 平台、库约束 5 平台(上游 227 已放宽 CN 未跟随) | **产品确认**:Nova composite 是否需要 CN 平台。需要→补 240 号迁移复刻上游 227+前端白名单加 CN;不需要→保持并记录 | 后端是否支持 CN composite 路由是关键;成本低(一迁移+白名单) |
