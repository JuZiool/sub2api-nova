...
const promptTokenCount = (row: Pick<AdminUsageLog, 'input_tokens' | 'cache_creation_tokens' | 'cache_read_tokens'>): number =>
  (row.input_tokens || 0) + (row.cache_creation_tokens || 0) + (row.cache_read_tokens || 0)

const formatCacheHitRate = (row: Pick<AdminUsageLog, 'input_tokens' | 'cache_creation_tokens' | 'cache_read_tokens'>): string => {
  const totalPromptTokens = promptTokenCount(row)
  return totalPromptTokens > 0
    ? ((row.cache_read_tokens || 0) / totalPromptTokens * 100).toFixed(1)
    : '0.0'
}
