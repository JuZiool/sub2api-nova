...
func RegisterGatewayRoutes(
	...
	hiddenModel := hiddenModelMiddleware()
	...
	r.POST("/images/generations/async", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), hiddenModel, compositeTarget, requireGroupAnthropic, h.AsyncImage.Submit)
	r.POST("/images/edits/async", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), hiddenModel, compositeTarget, requireGroupAnthropic, h.AsyncImage.Submit)
	...
	r.POST("/videos", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), hiddenModel, compositeTarget, requireGroupAnthropic, videoGenerationHandler)
	r.POST("/videos/generations", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), hiddenModel, compositeTarget, requireGroupAnthropic, videoGenerationHandler)
	r.POST("/videos/edits", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), hiddenModel, compositeTarget, requireGroupAnthropic, videoEditHandler)
	r.POST("/videos/extensions", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), hiddenModel, compositeTarget, requireGroupAnthropic, videoExtensionHandler)
	...
	antigravityV1Beta.Use(hiddenModel)
...
// hiddenModelMiddleware enforces group model deny rules before composite route
// rewriting can replace the public model with an upstream model.
func hiddenModelMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		apiKey, ok := middleware.GetAPIKeyFromContext(c)
		...
		if apiKey.Group.IsModelHidden(model) {
			service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalPolicyDenied)
			c.AbortWithStatusJSON(http.StatusForbidden, gin.H{
				"error": gin.H{
					"type":    "permission_error",
					"message": "The requested model is not available for this group",
				},
			})
			return
		}
