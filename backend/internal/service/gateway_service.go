...
type ForwardResult struct {
	...
	// RequestedReasoningEffort retains the client's value before policy or
	// model-family rewriting. It is audit metadata and never affects billing.
...
func (s *GatewayService) initDebugGatewayBodyFile(path string) {
	...
	if info, err := os.Stat(path); err == nil && info.IsDir() { //nolint:gosec // The debug log path is an explicit operator-provided diagnostic setting.
		path = filepath.Join(path, debugGatewayBodyDefaultFilename)
	}

	// 确保父目录存在
	if dir := filepath.Dir(path); dir != "." {
		if err := os.MkdirAll(dir, 0755); err != nil { //nolint:gosec // The debug log directory comes from an explicit operator-provided diagnostic setting.
			slog.Error("failed to create gateway debug log directory", "dir", dir, "error", err)
			return
		}
	}

	f, err := os.OpenFile(path, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0644) //nolint:gosec // The debug log path is an explicit operator-provided diagnostic setting.
	if err != nil {
