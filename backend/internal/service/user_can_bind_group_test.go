...
func TestUserCanBindGroup(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name:    "a restricted user keeps listed public groups",
			user:    User{RestrictPublicGroups: true, AllowedGroups: []int64{publicGroupA}},
			groupID: publicGroupA,
			want:    true,
		},
		{
			name:    "a restricted user loses unlisted public groups",
			user:    User{RestrictPublicGroups: true, AllowedGroups: []int64{publicGroupA}},
		...
		{
			name:        "a restricted user can hold both kinds of grant",
