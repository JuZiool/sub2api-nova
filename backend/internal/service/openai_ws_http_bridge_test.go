...
func TestPrepareOpenAIWSHTTPBridgeBodyStripsNoneReasoningForCompatibleEndpoint(t *testing.T) {
	payload := []byte(`{"type":"response.create","model":"company-coding-model","reasoning":{"effort":"none"},"reasoning_effort":"NONE","input":"hi"}`)
	...
	require.Equal(t, "NONE", gjson.GetBytes(officialBody, "reasoning_effort").String())
}
...
func TestOpenAIWSPassthroughFirstMessageBridgeDecision(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "oversized response create bridges", payload: `{"type":"response.create","input":"` + strings.Repeat("x", 100) + `"}`, want: true},
		{name: "omitted type bridges", payload: `{"input":"` + strings.Repeat("x", 100) + `"}`, want: true},
		{name: "blank type bridges", payload: `{"type":"   ","padding":"` + strings.Repeat("x", 100) + `"}`, want: true},
		...
		{name: "other event stays passthrough", payload: `{"type":"session.update","padding":"` + strings.Repeat("x", 100) + `"}`},
		{name: "malformed JSON stays passthrough", payload: `{"type":"response.create","padding":"` + strings.Repeat("x", 100)},
	}
	...
	disabledSvc := &OpenAIGatewayService{cfg: &config.Config{Gateway: config.GatewayConfig{
		OpenAIWS: config.GatewayOpenAIWSConfig{
			HTTPBridgeThresholdBytes: threshold,
		},
	}}}
	require.False(t, disabledSvc.shouldBridgeOpenAIWSPassthroughFirstMessage(nil, []byte(tests[0].payload)))
}
