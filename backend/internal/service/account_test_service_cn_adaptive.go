...
// APIProtocolAdaptive is kept local to the account-test implementation because
// this branch predates the shared adaptive protocol constant.
const APIProtocolAdaptive = "adaptive"

func accountTestAPIProtocol(account *Account) string {
	if account == nil {
		return ""
	}
	if !account.IsCNProvider() {
		return account.GetAPIProtocol()
	}
	switch strings.TrimSpace(account.GetCredential("api_protocol")) {
...
// accountTestCNProtocolBaseURL resolves the per-protocol upstream URL needed by
// account tests. It is kept private to the account-test implementation because
// this branch predates the shared adaptive protocol helpers.
func accountTestCNProtocolBaseURL(a *Account, protocol string) string {
	if a == nil || !a.IsCNProvider() {
		return ""
	}
	if accountTestAPIProtocol(a) == APIProtocolAdaptive {
	...
	} else if baseURL := strings.TrimSpace(a.GetCredential("base_url")); baseURL != "" {
		return baseURL
	}
...
func (s *AccountTestService) testCNProviderChatCompletionsConnection(c *gin.Context, account *Account, modelID string, prompt string) error {
	...
	baseURL := accountTestCNProtocolBaseURL(account, APIProtocolChatCompletions)
...
func (s *AccountTestService) testCNProviderAdaptiveAnthropicConnection(c *gin.Context, account *Account, testModelID string, authToken string) error {
	...
	baseURL, err := s.validateUpstreamBaseURL(accountTestCNProtocolBaseURL(account, APIProtocolAnthropic))
	if err != nil {
...
func (s *AccountTestService) testCNProviderAdaptiveResponsesConnection(c *gin.Context, account *Account, testModelID string, authToken string) error {
	ctx := c.Request.Context()
	baseURL, err := s.validateUpstreamBaseURL(accountTestCNProtocolBaseURL(account, APIProtocolResponses))
	if err != nil {
