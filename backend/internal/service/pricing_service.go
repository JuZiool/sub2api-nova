...
func (s *PricingService) parsePricingData(body []byte) (map[string]*LiteLLMModelPricing, error) {
	...
	for modelName, rawEntry := range rawData {
		...
		pricing := &LiteLLMModelPricing{
			...
			LongContextThresholdInclusive: strings.EqualFold(strings.TrimSpace(entry.LiteLLMProvider), "xai"),
			LongContextExplicit:           entry.LongContextInputTokenThreshold != nil || entry.LongContextInputCostMultiplier != nil || entry.LongContextOutputCostMultiplier != nil,
		}
		...
		} else if pricing.LongContextInputTokenThreshold > 0 {
...
func (s *PricingService) useFallbackPricing() error {
	...
	pricingFile := s.getPricingFilePath()
	if err := os.WriteFile(pricingFile, data, 0644); err != nil { //nolint:gosec // The pricing file path is derived from the validated application data directory.
