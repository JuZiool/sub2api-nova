...
type ModelPricing struct {
	...
	LongContextExplicit                bool     // 源数据显式提供 long_context 字段，允许 0 关闭阶梯
...
/* configuredServiceTierMultiplier 优先使用渠道显式配置的 Fast/Flex 倍率，未配置时回退默认档位倍率。 */
...
func (s *BillingService) computeTokenBreakdown(
	...
	if longContextPricingEligible {
		...
		// 倍率按 orOne 归一：目录单侧漏配 0 时按 1x，避免对应分项免费（见 #2293 同类问题）。
...
func (s *BillingService) applyModelSpecificPricingPolicy(model string, pricing *ModelPricing) *ModelPricing {
	...
	if pricing.LongContextExplicit {
