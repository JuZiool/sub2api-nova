...
var usageLogInsertArgTypes = [...]string{
	...
	"text",        // rate_rule_source
	"text",        // rate_rule_key
	"bigint",      // rate_config_version
	"numeric",     // rate_base_multiplier
	"numeric",     // rate_token_multiplier
...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	rateRuleSource := nullString(log.RateRuleSource)
	rateRuleKey := nullString(log.RateRuleKey)
	rateConfigVersion := nullInt64(log.RateConfigVersion)
	...
	return usageLogInsertPrepared{
		...
		args: []any{
			...
			rateRuleKey,
			rateConfigVersion,
			log.RateBaseMultiplier,
			log.RateTokenMultiplier,
