...
func TestPrepareUsageLogInsert_UpstreamRequestIDArgWiring(t *testing.T) {
	...
	// Nova 在 session_id 之后追加了 pricing_group_id 与 rate_* 等 10 列（列尾为
	// created_at、requested_reasoning_effort、native_compaction_v2），
	// upstream_request_id 仍紧跟 session_id，因此从尾部数第 14 位。
	idx := len(prepared.args) - 14
