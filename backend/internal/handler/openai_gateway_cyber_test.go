...
// TestRecordCyberPolicyIfMarked_BlockBodyPlumbed verifies the request body is
// accepted and a non-empty body with nil gateway service does not panic
// (write-side guards live in the service layer).
func TestRecordCyberPolicyIfMarked_BlockBodyPlumbed(t *testing.T) {
	c := newTestGinContext()
	service.MarkOpsCyberPolicy(c, service.CyberPolicyMark{Message: "x", UpstreamStatus: 400})
	h := &OpenAIGatewayHandler{}
	require.NotPanics(t, func() {
		h.recordCyberPolicyIfMarked(c, nil, nil, nil, "gpt-5", true, []byte(`{"prompt_cache_key":"deadbeef"}`), service.ChannelUsageFields{}, "")
	})
}
