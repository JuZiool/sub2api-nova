...
// does not affect upstream account health or scheduling.
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	if apiKey.Group != nil && apiKey.Group.IsModelHidden(reqModel) {
	...
	// F5a: 首包会话屏蔽检查。首包已在握手前读取，显式标识与 transcript
	// 屏蔽都可以在账号选择前判定；连接内 flag 负责拦截后续 turn。
	...
	for {
		...
		hooks := &service.OpenAIWSIngressHooks{
			...
			BeforeRequest: func(turn int, payload []byte, originalModel string) error {
				...
				if apiKey.Group != nil && apiKey.Group.IsModelHidden(model) {
